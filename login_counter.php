...
if (isset($_SESSION['username'])) {
    ...
    $stmt = $conn->prepare("SELECT login FROM tlogin WHERE username = ?");
    ...
    $update_stmt = $conn->prepare("UPDATE tlogin SET login = ? WHERE username = ?");
...
function display_user_info($username, $display_counter) {
    ...
    echo "<script>
        var userInfo = 'Welcome, ' + '<i>' + '$username' + '</i>' + '! You have visited this page ' + '<strong>' + '$display_counter' + '</strong>' + ' times.';
