...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!in_array($fileExtension, $allowedExtensions)) {
      ...
      if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
        ...
        while (($row = fgetcsv($fileHandle, 1000, ',')) !== false) {
            ...
            $mask           = isset($row[1]) ? $row[1] : null;
            $status         = isset($row[2]) ? $row[2] : null;
            $city           = isset($row[3]) ? $row[3] : null;
            $building       = isset($row[4]) ? $row[4] : null;
            $floor          = isset($row[5]) ? $row[5] : null;
            $net_type       = isset($row[6]) ? $row[6] : null;
            $subnet_type    = isset($row[7]) ? $row[7] : null;

            // Check if required fields are missing
            if (empty($ipAddress) || empty($mask) || empty($status) || empty($city) || empty($building) || empty($floor) || empty($net_type) || empty($subnet_type)) {
            ...
            $sql = "INSERT INTO `ipam` (`IPADDRESS`, `MASK`, `STATUS`, `CITY`, `BUILDING`, `FLOOR`, `NET_TYPE`, `SUBNET_TYPE`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

            try {
                $stmt = $conn3->prepare($sql);
                $stmt->bind_param('ssssssss', $ipAddress, $mask, $status, $city, $building, $floor, $net_type, $subnet_type);
  ...
  if ($errorDetected) {
    ...
    header("Location: ipam.php?msg=" . urlencode($errorMessage));
    exit();
  }

  // Redirect to index.php with success message
  header("Location: ipam.php?msg=" . urlencode($successMessage));
  exit();
}
